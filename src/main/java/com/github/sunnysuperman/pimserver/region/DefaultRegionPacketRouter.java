...
public class DefaultRegionPacketRouter extends PacketSafeSenderBuilder implements RegionPacketRouter {
	...
	private class RegionClientHandler extends PacketHandler {
			...
			try {
				...
				case PacketType.TYPE_PING: {
					...
					if (currentChannel != contextChannel) {
						LOG.warn("No the same region connection, so do not reply pong, maybe previous connection is not closed properly: "
	...
	public DefaultRegionPacketRouter(RegionClientConfig config) {
		...
		this.senders = new ConcurrentHashMap<String, PacketSafeSender>(config.getRegions().size() - 1);
		...
		config.addListener(new ConfigValueChangedListener() {
			...
			public void onChanged(String key, Object value) {
				if (key.equals("regions")) {
					...
					synchronized (LOCK) {
						for (Iterator<Entry<String, PacketSafeSender>> iter = senders.entrySet().iterator(); iter
								.hasNext();) {
	...
	private boolean ensureSenders() {
		...
		synchronized (LOCK) {
			try {
				// 删除无效Connection
				for (Iterator<Entry<String, PacketSafeSender>> iter = senders.entrySet().iterator(); iter.hasNext();) {
					Entry<String, PacketSafeSender> entry = iter.next();
	...
	@Override
	public boolean routeToRegion(String regionId, CommunicationPacket packet, int maxTry) {
